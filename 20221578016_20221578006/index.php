...
<html>
    ...
    <body>
        ...
        $totalAPagar = 0;
        $cantidad = 0;
        $totalDescuentosPorUnidad = 0;
        $TotalIVA = 0;
        $totalDescuentoPorVenta = 0;
        $totalSubtotales = 0;
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $totalProductoConDescuento = ($cantidad*$valoUnitario) - ($descuentoPorUnidad*$cantidad);
            $totalAPagar += ($totalProductoConDescuento);
        ...
        if (!empty($preciosProductos)) {
            ...
            foreach ($productos_capturados as $producto) {
                ...
                $totalSubtotales += $producto['Total con Descuento'];
                $totalDescuentoPorVenta += $producto['Descuento * Unidad']* $cantidad;
            }
            $NetoAPagar = $totalSubtotales+$TotalIVA;
            echo "</tbody></table>";
            ...
            echo "<br><label>Total Descuento por venta: <input type=number value=$totalDescuentoPorVenta></label><br/>";
            echo "</div>";
        } else {
